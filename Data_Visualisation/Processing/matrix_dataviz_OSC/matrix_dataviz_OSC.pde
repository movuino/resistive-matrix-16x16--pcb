...
void setup()
{
  ...
  // OSC communication
  initMovuinoOSC("127.0.0.1", 3000, 3001);
...
void draw()
{
  ...
  for (int i = 0; i < ROWS; i++) {
    ...
    for (int j = 0; j < COLS; j++) {
      ...
      //pointGrid[i][j].curRemapVal 4095.0; // 12-bit ADC (for full range values)
      pointGrid[i][j].curRemapVal /= 255.0; // 8-bit ADC values
      ...
      if(isRecording){
        if (i == 0){
          newRow = table.addRow();
          newRow.setFloat("time",(float)(time_sec - millis())/1000);
        }
